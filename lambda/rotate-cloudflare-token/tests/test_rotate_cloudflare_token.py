...
class TestCloudflareTokenRotation(unittest.TestCase):
    ...
    def setUp(self):
        self.secret_arn = (
            "arn:aws:secretsmanager:us-east-1:123456789012:secret:cloudflare-test"
        )
    ...
    def test_create_secret_success(self, mock_urlopen, mock_secrets_client):
        ...
        mock_secrets_client.get_secret_value.side_effect = [
            ...
            ),  # Idempotency check fails (means clean run)
            {
                "SecretString": self.mock_secret_string
            },  # Fetching AWSCURRENT configuration
        ]
                    ...
                    "policies": [
                        {
                            "effect": "allow",
                            "resources": {},
                            "permission_groups": [{"id": "dns-write-id"}],
                        }
                    ]
                },
            }
    ...
    def test_test_secret_success(self, mock_urlopen, mock_secrets_client):
        ...
        mock_secrets_client.get_secret_value.assert_called_once_with(
            SecretId=self.secret_arn,
            VersionId=self.token_uuid,
            VersionStage="AWSPENDING",
        )

    @patch("rotate_cloudflare_token.client")
    @patch("urllib.request.urlopen")
    def test_finish_secret_with_successful_cleanup(
        self, mock_urlopen, mock_secrets_client
    ):
        """Test finish_secret promotion and cleanup of the old token."""
        ...
        mock_delete_resp.read.return_value = json.dumps({"success": True}).encode(
            "utf-8"
        )
